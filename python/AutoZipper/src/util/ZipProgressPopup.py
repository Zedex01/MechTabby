...
class ZipProgressPopup(ctk.CTkToplevel):
    def __init__(self, master, paths):
        ...
        self.label = ctk.CTkLabel(self, text=f"Zipping")
    ...
    def zip_files(self, paths, volumes=False, size=1000):
        sz = SevenZip(paths)
        if volumes:
            sz.set_volumes=True
            sz.set_volume_size_mb=size
